...
Writes:  data/{year}/{event}/{session}/session_meta.json  (metadata + driver list, ~5KB)
         data/{year}/{event}/{session}/{DRIVER}.json       (per-driver laps+telemetry)
Usage: python tel.py <year> <event> <session>
"""
import sys, json, fastf1
import pandas as pd
...
def extract_session(year: int, event: str, session_type: str):
    ...
    if hasattr(session, "track_status") and session.track_status is not None:
        for _, row in session.track_status.iterrows():
            try:
                t = row["Time"].total_seconds() if hasattr(row["Time"], "total_seconds") else float(row["Time"])
    ...
    base_dir = Path(__file__).parent.parent / "data" / str(year) / event / session_type
    base_dir.mkdir(parents=True, exist_ok=True)

    driver_colors = {}
    for drv in drivers:
        driver_colors[drv] = driver_color(drv, year)

    # ── Write session metadata (small file) ──────────────────────────────
    meta = {
        "year":           year,
        "event":          event,
    ...
    write_json(base_dir / "session_meta.json", meta)
    ...
    for drv in drivers:
            ...
            try:
                ...
                if tel is not None and not tel.empty:
                    for _, t in tel.iterrows():
                        ...
                        if aero["DRS"] is not None:
                            point["drs"] = safe_int(aero["DRS"])
                        else:
                            point["aa"] = safe_int(aero["ActiveAero"])
                        tel_data.append(point)
        ...
        write_json(base_dir / f"{drv}.json", driver_data)
        print(f"  wrote {drv} ({len(laps_list)} laps, {sum(len(l['tel']) for l in laps_list)} tel points)")

    print(f"Done: {base_dir}  ({len(drivers)} drivers)")
